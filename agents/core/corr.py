...
try:
    ...
    print("Warning: corr_sampler CUDA extension not available, falling back to PyTorch implementation")

try:
    import alt_cuda_corr
    ALT_CUDA_CORR_AVAILABLE = True
except:
    # alt_cuda_corr is not compiled
    ALT_CUDA_CORR_AVAILABLE = False
...
class CorrSampler(torch.autograd.Function):
    ...
    def forward(ctx, volume, coords, radius):
        if not CORR_SAMPLER_AVAILABLE:
            ...
            batch, h1, w1, w2 = volume.shape
            coords = coords.permute(0, 2, 3, 1)  # [B, H, W, 1]
            
            # Create sampling grid around each coordinate - ensure same dtype as volume
            dx = torch.linspace(-radius, radius, 2*radius+1, device=coords.device, dtype=coords.dtype)
            dx = dx.view(1, 1, 1, 2*radius+1, 1)
            
            # Broadcast coordinates
            coords = coords.unsqueeze(-2)  # [B, H, W, 1, 1]
            sample_coords = coords + dx   # [B, H, W, 2*radius+1, 1]
            
            # Normalize coordinates to [-1, 1] for grid_sample
            sample_coords = 2 * sample_coords / (w2 - 1) - 1
            zeros = torch.zeros_like(sample_coords)
            grid = torch.cat([sample_coords, zeros], dim=-1)  # [B, H, W, 2*radius+1, 2]
            
            # Reshape for grid_sample
            B, H, W, K, _ = grid.shape
            grid = grid.view(B, H*W, K, 2)
            volume_reshaped = volume.view(B, h1*w1, 1, w2).permute(0, 3, 1, 2)
            
            # Ensure volume_reshaped and grid have the same dtype
            if volume_reshaped.dtype != grid.dtype:
                grid = grid.to(volume_reshaped.dtype)
            
            # Sample
            sampled = F.grid_sample(volume_reshaped, grid, align_corners=True, mode='bilinear', padding_mode='zeros')
            corr = sampled.permute(0, 2, 3, 1).view(B, H, W, K)
    ...
    def backward(ctx, grad_output):
        if not CORR_SAMPLER_AVAILABLE:
            # Simple gradient passthrough for fallback
...
class AlternateCorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        if not ALT_CUDA_CORR_AVAILABLE:
            raise NotImplementedError("AlternateCorrBlock requires alt_cuda_corr extension which is not available")
        self.num_levels = num_levels
        self.radius = radius
