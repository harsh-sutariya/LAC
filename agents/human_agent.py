...
class HumanAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        os.makedirs('live_trajectory/mav0/cam0/data', exist_ok=True)
        os.makedirs('live_trajectory/mav0/cam1/data', exist_ok=True)
        
        # Initialize timestamp file for ORB-SLAM3
        self._timestamp_file = 'live_trajectory/mav0/timestamps.txt'
        self._start_time = datetime.datetime.now()
        self._frame_rate = 20.0  # 20 FPS (adjust based on your simulation)
        
        # Create and initialize timestamp file
        with open(self._timestamp_file, 'w') as f:
            f.write("# ORB-SLAM3 Timestamp File\n")
            f.write("# Format: timestamp left_image right_image\n")
            f.write("# Generated by HumanAgent for synchronized stereo SLAM\n\n")
        
        print("Stereo image saving with timestamp synchronization enabled!")
        print(f"  - Images saved to: live_trajectory/mav0/cam*/data/")
        print(f"  - Timestamps saved to: {self._timestamp_file}")
        print(f"  - Frame rate: {self._frame_rate} FPS")
        print(f"  - Format: synchronized left_N.png ↔ right_N.png pairs")
    ...
    def _save_stereo_images_step(self, input_data):
        """
        Save left and right stereo images with synchronized timestamps for ORB-SLAM3
        """
        ...
        if left_img is not None and right_img is not None:
            ...
            left_dir = './live_trajectory/mav0/cam0/data'
            right_dir = './live_trajectory/mav0/cam1/data'
            os.makedirs(left_dir, exist_ok=True)
            os.makedirs(right_dir, exist_ok=True)

            # Calculate timestamp based on frame number and frame rate
            timestamp = self._start_time.timestamp() + (self._image_counter / self._frame_rate)
            
            # Build synchronized filenames (same frame number for both cameras)
            ...
            # Append to timestamp file for ORB-SLAM3
            with open(self._timestamp_file, 'a') as f:
                f.write(f"{timestamp:.9f} left_{self._image_counter:03d}.png right_{self._image_counter:03d}.png\n")

            # Print progress every 10 frames to avoid spam
            if self._image_counter % 10 == 0:
                print(f"[Stereo Save] Frame {self._image_counter:03d}: {left_filename} ↔ {right_filename} (t={timestamp:.3f}s)")
            ...
            if self._image_counter == 0:  # Only print once
                ...
                print("Use keys 2 and 3 to activate FrontLeft and FrontRight cameras respectively.")
                print("Or use keys 4 and 5 to activate Left and Right cameras respectively.")
    ...
    def finalize(self):
        ...
        # Print final statistics and create summary
        if hasattr(self, '_image_counter') and self._image_counter > 0:
            print(f"\n🎉 Stereo image saving complete!")
            print(f"  📊 Total stereo pairs saved: {self._image_counter}")
            print(f"  📁 Images saved to: live_trajectory/mav0/cam*/data/")
            print(f"  ⏰ Timestamps saved to: {self._timestamp_file}")
            print(f"  🎬 Frame rate: {self._frame_rate} FPS")
            print(f"  ⏱️  Duration: {self._image_counter / self._frame_rate:.1f} seconds")
            
            # Create a summary file for ORB-SLAM3
            summary_file = 'live_trajectory/mav0/summary.txt'
            with open(summary_file, 'w') as f:
                f.write(f"ORB-SLAM3 Dataset Summary\n")
                f.write(f"========================\n")
                f.write(f"Total stereo pairs: {self._image_counter}\n")
                f.write(f"Frame rate: {self._frame_rate} FPS\n")
                f.write(f"Duration: {self._image_counter / self._frame_rate:.1f} seconds\n")
                f.write(f"Timestamp file: timestamps.txt\n")
                f.write(f"Left camera: cam0/data/left_XXX.png\n")
                f.write(f"Right camera: cam1/data/right_XXX.png\n")
                f.write(f"\nORB-SLAM3 Command:\n")
                f.write(f"cd ORB_SLAM3\n")
                f.write(f"./Examples/Stereo/stereo_euroc Vocabulary/ORBvoc.txt Examples/Stereo/EuRoC.yaml \\\n")
                f.write(f"    ../live_trajectory/mav0/cam0/data ../live_trajectory/mav0/cam1/data \\\n")
                f.write(f"    ../live_trajectory/mav0/timestamps.txt output_trajectory\n")
            
            print(f"  📋 Summary saved to: {summary_file}")
            print(f"  🚀 Ready for ORB-SLAM3 processing!")
        else:
            print("\n⚠️  No stereo images were saved. Check camera activation!")
