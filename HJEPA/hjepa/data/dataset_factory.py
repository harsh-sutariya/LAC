...
from hjepa.data.lunar_rover import LunarRoverDataset
...
class DatasetFactory:
    ...
    def create_datasets(self):
        ...
        elif self.config.dataset_type == DatasetType.LunarRover:
            return self._create_lunar_rover_datasets()
        else:
    ...
    def _create_d4rl_datasets(self):
        ...
        if self.disable_l2:
            ...
            datasets = Datasets(
                ...
                val_ds=None,
                probing_datasets=ProbingDatasets(ds=probe_ds, val_ds=probe_val_ds),
                l2_probing_datasets=ProbingDatasets(ds=probe_ds, val_ds=probe_val_ds),
            )
    ...
    def _create_lunar_rover_datasets(self):
        """Create lunar rover navigation datasets"""
        from hjepa.data.utils import make_dataloader
        
        # Create training dataset
        ds = LunarRoverDataset(self.config.lunar_rover_config)
        ds = make_dataloader(
            ds=ds, 
            loader_config=self.config, 
            suffix="lunar_rover"
        ...
        val_config = dataclasses.replace(
            self.config.lunar_rover_config,
            train=False,
            quick_debug=self.config.quick_debug
        )
        val_ds = LunarRoverDataset(val_config, normalizer=ds.normalizer)
        val_ds = make_dataloader(
            ds=val_ds,
            loader_config=self.config,
            suffix="lunar_rover_val"
        )
        
        datasets = Datasets(
            ds=ds,
            val_ds=val_ds,
            probing_datasets=None,  # Not implemented for lunar rover yet
            l2_probing_datasets=None,
            aae_dataset=None
        )
