...
class DiscreteNet(torch.nn.Module):
    ...
    def sample(self, logits, argmax=False):
        ...
        if argmax:
            ...
            samples = F.one_hot(argmax_inds, num_classes=self.latent_dist_size).to(
    ...
    def __init__(
        ...
        if arch == "conv":
            ...
            self.posterior_net = MLP(
                arch=arch,
                input_dim=math.prod(input_dim) * 2,
    ...
    def forward(
        ...
        if self.posterior_input_type == "term_states":
            if self.arch == "conv":
                ...
                x = torch.cat(
                    [
                        flatten_conv_output(x_prev),
                        flatten_conv_output(x_next),
                    ],
                    dim=-1,
                )
        else:
        ...
        mu, std = self.posterior_net(x).chunk(2, dim=-1)
        std = F.softplus(std) + self.min_std
        return mu, std
