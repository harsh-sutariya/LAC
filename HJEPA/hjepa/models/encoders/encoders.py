...
class MeNet6(SequenceBackbone):
    ...
    def _build_proprio_encoder(
        self, arch: str, input_dim, proprio_dim: int, fuse: bool
    ):
...
def build_backbone(
    ...
    elif arch == "lunar_rover":
        from hjepa.models.encoders.lunar_rover import LunarRoverEncoder
        backbone = LunarRoverEncoder(
            config=config,
            input_dim=input_dim,
            input_proprio_dim=input_proprio_dim,
            input_loc_dim=input_loc_dim,
            normalizer=normalizer,
            imu_encoder_path=getattr(config, 'imu_encoder_path', None),
            imu_scaler_path=getattr(config, 'imu_scaler_path', None),
            image_size=getattr(config, 'image_size', (256, 256)),
            freeze_vision_encoder=getattr(config, 'freeze_vision_encoder', True),
            freeze_imu_encoder=getattr(config, 'freeze_imu_encoder', True),
        )
    elif l2:
