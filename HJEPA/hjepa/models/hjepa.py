...
class HJEPA(torch.nn.Module):
    def __init__(
        ...
        if not self.config.disable_l2:
            self.level2 = JEPA(
                config.level2,
                input_dim=self.level1.backbone.output_dim,
                input_obs_dim=self.level1.backbone.output_obs_dim,
                input_proprio_dim=self.level1.backbone.output_proprio_dim,
    ...
    def forward_posterior(
        ...
        if not self.config.disable_l2:
            ...
            forward_result_l2 = self.level2.forward_posterior(
                encodings, actions=None, goal=goal
            )
        else:
